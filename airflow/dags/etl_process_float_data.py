...
with DAG(
    ...
    download_sheet_task = PythonOperator(
        task_id="download_float_sheet",
        python_callable=lambda: download_google_sheet(CSV_URL),
    )

    transform_task = PythonOperator(
        task_id="transform_float_data",
        python_callable=lambda ti: transform_float_data(
            ti.xcom_pull(task_ids="download_float_sheet")
        ),
    )

    ingest_to_postgres_task = PythonOperator(
        task_id="ingest_float_data_to_postgres",
        python_callable=lambda ti: ingest_to_postgres(
            ti.xcom_pull(task_ids="transform_float_data"),
            USER,
            PASSWORD,
            HOST,
            PORT,
